...
// Assert : 검증을 통해서 코드가 통과하는지 조사. 조건이 안 맞으면 코드가 중단!!
//          디버깅용으로 많이 쓰임

var myInt : Int = 10
// 10이면 통과, 아니면 중단
//assert(myInt == 10, "myInt가 10이 아니다.") // 중단!!

myInt = 20
//assert(myInt == 10, "myInt가 10이 아니다.") // 중단!!

func myFuncAssert(age:Int?) {
    assert(age != nil, "age가 nil")
    assert(age! >= 0 && age! < 120, "age가 정상범위를 벗어남")
